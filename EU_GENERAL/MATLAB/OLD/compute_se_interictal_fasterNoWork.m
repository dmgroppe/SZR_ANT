...
for cloop=1:n_chan,
    if cloop>1
        chan_ptrs(cloop)=total_n_nonszr_winds;
    end
...
for floop=1:n_files,
    ...
    for cloop=1:n_chan,
        fprintf('Working on chan %s-%s (Channel %d/%d)\n',soz_chans_bi{cloop,1}, ...
            soz_chans_bi{cloop,1},cloop,n_chan);
        fprintf('Using file %d/%d %s\n',floop,n_files,file_info(floop).fname);
        
        % Randomly selected subset of non-szr features
        use_non_szr_ids=non_szr_ids(randi(length(non_szr_ids),1,n_nonszr_obs(cloop)));
        
        if isempty(nonszr_se_ftrs),
            %preallocate mem
            nonszr_se_ftrs=zeros(n_ftrs,total_n_nonszr_winds);
            nonszr_se_ftrs_time_sec=zeros(1,total_n_nonszr_winds);
