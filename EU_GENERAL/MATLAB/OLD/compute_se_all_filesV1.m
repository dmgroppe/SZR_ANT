...
for floop=1:n_files,
    ...
    ieeg_soz=[];
    % Load and downsample (if necessary) raw data
    for cloop=1:n_chan,
        fprintf('Working on chan %s-%s\n',soz_chans_bi{cloop,1},soz_chans_bi{cloop,1});
        ...
        if cloop==1,
            ...
            n_ftrs_per_chan=n_lags*n_bands;
            se_ftrs=zeros(n_ftrs_per_chan*n_chan,n_ftr_wind);
        end
        
        % Compute raw feature without any smoothing
        ftr_offset=(cloop-1)*n_ftrs_per_chan;
        for bloop=1:n_bands,
            ...
            [se_ftrs(bloop+ftr_offset,:), hilb_ifreq]=bp_hilb_mag(bp_ieeg,n_ftr_wind,wind_len,wind_step);
        end
        
        % Set initial value of all features
        se_ftr_ids=[1:n_ftrs_per_chan]+ftr_offset;
        se_ftrs(se_ftr_ids,1)=repmat(se_ftrs([1:n_bands]+ftr_offset,1),n_lags,1);
        
        % Apply EDM smoothing
        fprintf('Applying EDM smoothing...\n');
        ftr_ids=1:n_bands;
        base_ftr_ids=1:n_bands+ftr_offset; % ?? PICKUP HERE
        for edm_loop=2:n_lags,
            ftr_ids=ftr_ids+n_bands;
            now_wt=1/(2^edm_lags(edm_loop));
            for t=2:n_ftr_wind,
                se_ftrs(ftr_ids+ftr_offset,t)=now_wt*se_ftrs(base_ftr_ids+ftr_offset,t)+...
                    (1-now_wt)*se_ftrs(ftr_ids+ftr_offset,t-1);
            end
        end
