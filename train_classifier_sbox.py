...
valid_sens=np.zeros(n_train_subs)
valid_spec=np.zeros(n_train_subs)
valid_acc=np.zeros(n_train_subs)
valid_bal_acc=np.zeros(n_train_subs)
train_sens=np.zeros(n_train_subs)
train_spec=np.zeros(n_train_subs)
train_acc=np.zeros(n_train_subs)
train_bal_acc=np.zeros(n_train_subs)
for left_out_id in range(n_train_subs):
    ...
    #rbf_svc.fit(ftrs[sub_id!=left_out_id,:], szr_class[sub_id!=left_out_id]) # TODO revert! ??
    rbf_svc.fit(ftrs[sub_id == 0, :], szr_class[sub_id == 0])
    #clf = svm.SVC()
    # >>> clf.fit(X, y)
    ...
    train_acc[left_out_id]=np.mean(jive[train_bool])
    print('Training accuracy: %f' % train_acc[left_out_id])
    use_ids=np.multiply(train_bool,ictal_bool)
    train_sens[left_out_id]=np.mean(jive[use_ids])
    print('Training sensitivity: %f' % train_sens[left_out_id])
    use_ids=np.multiply(train_bool,preictal_bool)
    train_spec[left_out_id]=np.mean(jive[use_ids])
    print('Training specificity: %f' % train_spec[left_out_id])
    train_bal_acc[left_out_id]=(train_spec[left_out_id]+train_sens[left_out_id])/2
    print('Training balanced accuracy: %f' % train_bal_acc[left_out_id])

    # Validation Data Results
    valid_acc[left_out_id]=np.mean(jive[valid_bool])
    print('Validation accuracy: %f' % valid_acc[left_out_id])
    use_ids=np.multiply(valid_bool,ictal_bool)
    valid_sens[left_out_id]=np.mean(jive[use_ids])
    print('Validation sensitivity: %f' % valid_sens[left_out_id])
    use_ids=np.multiply(valid_bool,preictal_bool)
    valid_spec[left_out_id]=np.mean(jive[use_ids])
    print('Validation specificity: %f' % valid_spec[left_out_id])
    valid_bal_acc[left_out_id] = (valid_spec[left_out_id] + valid_sens[left_out_id]) / 2
    print('Validation balanced accuracy: %f' % valid_bal_acc[left_out_id])
...
# f=open('temp_class_results.pkl','wb')
# pickle.dump()
