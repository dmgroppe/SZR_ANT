% This script:
% 1. Loads all all GOOD channels of eeg data files that contain seizures
% (both clinical and subclinical)
% 2. Decimates data to 256 Hz
% 3. Identifies which time points contain szrs
% 4. Writes data to a mat file for use in Python
...
for a=1:n_szrs,
    if strcmpi(szr_csv{a,6},'Clinical')
        clin_szr(a)=1;
    end
    if isempty(szr_csv{a,4}) || isempty(szr_csv{a,2}),
        szr_onoff_sec(a,1)=-1;
        szr_onoff_sec(a,2)=-1;
    else
...
szr_onoff_sec=szr_onoff_sec(clin_szr>0,:);
...
%for floop=1:1,
for floop=1:n_szrs,
    if szr_data_file_ids(floop)>0,
        just_ieeg_fname=fdir(szr_data_file_ids(floop)).name;
        ...
        % Load szr classes
        szr_ids=find( (tpts_sec>=szr_onoff_sec(floop,1)).*(tpts_sec<=szr_onoff_sec(floop,2))); % all time points during szr
        ...
        szr_type=szr_csv{floop,6};
        save(fullfile(out_data_path,out_fname),'Fs','tpts_sec','start_ts','stop_ts','ieeg256','is_szr','use_chans','szr_type','just_ieeg_fname');
